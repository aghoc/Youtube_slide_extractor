...
YouTube에는 컨퍼런스 발표, 세미나, 강의, 제품 데모처럼 발표자가 슬라이드를 띄워놓고 설명하는 영상이 많이 올라옵니다. 이런 영상은 내용 자체도 유용하지만, 화면에 나오는 발표 슬라이드를 따로 모아두면 나중에 레퍼런스로 훨씬 빠르게 다시 볼 수 있습니다.

YouTube Slide Extractor는 이런 발표 영상에서 슬라이드처럼 보이는 장면을 자동으로 추출하는 작은 Python 도구입니다. 영상을 보면서 스크린샷을 하나씩 찍는 대신, 영상 안의 주요 슬라이드 화면을 찾아 이미지로 저장하고, 발표 자료처럼 넘겨볼 수 있는 PDF로 묶어줍니다.

내부적으로는 [`yt-dlp`](https://github.com/yt-dlp/yt-dlp)로 YouTube 영상을 내려받고, [`ffmpeg`](https://ffmpeg.org/)로 일정 간격의 프레임을 샘플링한 뒤, 빈 화면과 비슷한 중복 프레임을 걸러냅니다. 최종 결과는 번호가 붙은 JPEG 이미지들과 하나의 PDF 파일로 저장됩니다.

발표 자료를 복습하거나, 기술 발표의 다이어그램을 참고하거나, 녹화된 강의/세미나에서 핵심 슬라이드만 빠르게 모아보고 싶을 때 쓰기 좋습니다.
...
이 프로젝트는 내부적으로 [`yt-dlp`](https://github.com/yt-dlp/yt-dlp)를 호출해 YouTube 영상을 다운로드합니다. 슬라이드 추출을 실행하기 전에 `yt-dlp`가 정상 동작하는지 먼저 확인해보면 문제를 빠르게 분리할 수 있습니다.
...
이 프로젝트는 [`yt-dlp`](https://github.com/yt-dlp/yt-dlp)와 [`ffmpeg`](https://ffmpeg.org/)를 외부 명령어로 호출합니다. `yt-dlp` 프로젝트는 현재 Unlicense로 배포되며, `yt-dlp` 릴리스 파일에는 빌드 방식에 따라 다른 라이선스의 구성요소가 포함될 수 있습니다. `ffmpeg`의 라이선스도 설치한 빌드 구성에 따라 달라질 수 있습니다.
